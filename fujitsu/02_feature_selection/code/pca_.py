...
def pca_plot(pca):
	...
	plt.savefig('results/pca-plot.png')
	plt.show()
...
def pca_scatter(X, y):
	...
	plt.savefig('results/pca-scatter 3D.png')
	plt.show()

if __name__ == '__main__':
	data = ld.load_data('data/train_set.txt', n_features=None)
	target = ld.load_data('data/train_targets.txt')
	components_num = 3
	data_reduced, exp_variance_ratio, pca = pca_(data, components_num)
	
	# store data reduced
	wd.write_data('data/train_set_pca.txt', data_reduced)
	...
	pca_plot(pca)
	pca_scatter(data_reduced, target.ravel())
