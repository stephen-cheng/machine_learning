...
def newSim(app_id,stage_id,weights=None):
	...
	sql="select node,cpu_usage,mem_usage,ioWaitRatio,weighted_io,diskR_band,diskW_band,netS_band,netR_band from os where timestamp_ >=%d/1000 and timestamp_ <= %d/1000 group by node;" %(startTime,endTime)
	cur.execute(sql)
	res=cur.fetchall()
	nodes={}
	for r in res:
	...
	sql="select node,ipc,L2_MPKI,L3_MPKI,DTLB_MPKI,ITLB_MPKI,L1I_MPKI,MUL_Ratio,DIV_Ratio,FP_Ratio,LOAD_Ratio,STORE_Ratio,BR_Ratio from log where timestamp_ >=%d/1000 and timestamp_ <= %d/1000 group by node;" %(startTime,endTime)
	cur.execute(sql)
	res=cur.fetchall()
...
def normalarray(a,weights=None):
	...
	for i in a:
		res.append(float(i))
	return np.arctan(np.array(res)) * 2 / np.pi * w
	#OMG, it takes so long time to get the result
...
def weights():
	...
	#get std from os
	sql1="select std(cpu_usage),std(mem_usage),std(ioWaitRatio),std(weighted_io),std(diskR_band),std(diskW_band),std(netS_band),std(netR_band) from os"
	cur.execute(sql1)
	res1=cur.fetchall()
	r1=list(res1)
	for each in r1:
	...
	#get std from log
	sql2="select std(ipc),std(L2_MPKI),std(L3_MPKI),std(DTLB_MPKI),std(ITLB_MPKI),std(L1I_MPKI),std(MUL_Ratio),std(DIV_Ratio),std(FP_Ratio),std(LOAD_Ratio),std(STORE_Ratio),std(BR_Ratio) from log"
	cur.execute(sql2)
	res2=cur.fetchall()
	r2=list(res2)
	for each in r2:
