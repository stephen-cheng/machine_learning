...
def newSim(app_id,stage_id,weights=None):
	...
	for i in nodes:
		for j in nodes:
			sim[i][j]=sim_eud(normalarray(nodes[i],weights),normalarray(nodes[j],weights))*5/8 + sim_cos(normalarray(nodes[i],weights),normalarray(nodes[j],weights))*3/8
...
def draw_heat(xname,data):
	...
	map = ax1.imshow(data, interpolation="nearest", cmap=cmap, aspect='auto', vmin=0.8, vmax=1.0)
	...
	plt.savefig('node_sim_maxeu_heatmap.png')	
	plt.show() 
