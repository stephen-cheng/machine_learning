...
def nodeIndex(app_id,stage_id,weights):
	...
	sql = "select node, ipc, L2_MPKI, L3_MPKI, DTLB_MPKI, ITLB_MPKI, L1I_MPKI, MUL_Ratio, DIV_Ratio, FP_Ratio, LOAD_Ratio, STORE_Ratio, BR_Ratio from log where timestamp_ >=%d/1000 and timestamp_ <= %d/1000 and node = '%s';" %(startTime, endTime, node_list[3])
	cur.execute(sql)
	res = cur.fetchall()
	nodes = {}
	normal_list = []
...
def weights():
	...
	node_list = ["hw004", "hw089", "hw062", "hw073", "hw103", "hw114", "hw106"]
	sql2 = "select max(ipc), max(L2_MPKI), max(L3_MPKI), max(DTLB_MPKI), max(ITLB_MPKI), max(L1I_MPKI), max(MUL_Ratio), max(DIV_Ratio), max(FP_Ratio), max(LOAD_Ratio), max(STORE_Ratio), max(BR_Ratio) from log where timestamp_ >=%d/1000 and timestamp_ <= %d/1000 and node = '%s';" %(startTime, endTime, node_list[3])
	cur.execute(sql2)
	res2 = cur.fetchall()
	r2 = list(res2)
	for each in r2:
