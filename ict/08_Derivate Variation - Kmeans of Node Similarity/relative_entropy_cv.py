...
def c_rate(node,indexname,tablename,starttime,endtime,bins):
	conn=InsertTools.getConnection()
	cur=conn.cursor()
	sql="select %s from %s where node='%s' and timestamp_>=%s and timestamp_<%s;" %(indexname,tablename,node,starttime,endtime)
	cur.execute(sql)
	rows=cur.fetchall()
	node_values=[r[0] for r in rows]+list(np.linspace(0,1,bins))
	results = []
	for i in range(len(node_values)-1):
		rate = float(-node_values[i]+node_values[i+1])/node_values[i]
		results.append(rate)
...
if __name__=='__main__':
	...
	cr1 = c_rate(sys.argv[1],sys.argv[3],sys.argv[4],sys.argv[5],sys.argv[6],int(sys.argv[7]))
	cr2 = c_rate(sys.argv[2],sys.argv[3],sys.argv[4],sys.argv[5],sys.argv[6],int(sys.argv[7]))
	print (r1+r2)/2
	print "coefficient of variation:",cr1,cr2
