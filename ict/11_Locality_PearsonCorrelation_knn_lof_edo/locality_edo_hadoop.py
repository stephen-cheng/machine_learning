...
def node_data(stage_id):
	...
	sql="select hostname, locality, task_finishTime from task_attempt where stage_id='%s';" %(stage_id)
	cur.execute(sql)
	res=cur.fetchall()
	node=[]
	for r in res:
	...
	locality=['NODE_LOCAL','OFF_SWITCH', 'RACK_LOCAL']
...
if __name__=='__main__':
	stage_id=sys.argv[1]
	#stage_id = "mapStage_job_1493084522519_0014" 
	locality=['NODE_LOCAL','OFF_SWITCH', 'RACK_LOCAL']
	locality_weight=[0.0,0.0,1.0,2.0,3.0]
