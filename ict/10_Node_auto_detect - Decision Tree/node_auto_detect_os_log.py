...
import node_pca_os as npo
import node_pca_log as npl
...
if __name__=='__main__':
	...
	if node == []:
		...
		if len(outlier_sim) >= 2:
			for node_i in outlier_sim[1:]:
				data_os = npo.nodeIndex(stage_id, node_i)
				low_os, raw_os, pca_os = npo.pca(data_os, 8, 8)
				npo.plotBestFit(low_os,raw_os,node_i)
				pca_list_o.extend(pca_os)
			pca_list_os = list(set(pca_list_o))
			pca_list_os.sort(key=pca_list_o.index)
		else:
			data_os = npo.nodeIndex(stage_id, node[0])
			low_os, raw_os, pca_os = npo.pca(data_os, 8, 8)
			npo.plotBestFit(low_os,raw_os,node[0])
			pca_list_o.extend(pca_os)
			pca_list_os = list(set(pca_list_o))
			pca_list_os.sort(key=pca_list_o.index)
		#print "Recommended features as PCA in os: ", pca_list_os
		## pca of log features
		pca_list_l = []
		if len(outlier_sim) >= 2:
			for node_i in outlier_sim[1:]:
				data_log = npl.nodeIndex(stage_id, node_i)
				low_log, raw_log, pca_log = npl.pca(data_log, 12, 12)
				npl.plotBestFit(low_log,raw_log,node_i)
				pca_list_l.extend(pca_log)
			pca_list_log = list(set(pca_list_l))
			pca_list_log.sort(key=pca_list_l.index)
		else:
			data_log = npl.nodeIndex(stage_id, node[0])
			low_log, raw_log, pca_log = npl.pca(data_log, 12, 12)
			npl.plotBestFit(low_log,raw_log,node[0])
			pca_list_l.extend(pca_log)
			pca_list_log = list(set(pca_list_l))
			pca_list_log.sort(key=pca_list_l.index)
		#print "Recommended features as PCA in log: ", pca_list_log
		
		## print results into file
		file = open('file/%s_results.txt' % (stage_id),'a')
		file.write("Recommended features as PCA in os: "+"\r\n")
		file.write(" ".join(pca_list_os)+"\r\n")
		file.write("Recommended features as PCA in log: "+"\r\n")
		file.write(" ".join(pca_list_log)+"\r\n")
		file.close()
		
		
		##————————similarity analysis of all nodes on single feature by mean knn————————##
		#feature_list_os = ['cpu_usage', 'ioWaitRatio', 'weighted_io', 'mem_usage', 'diskR_band', 'diskW_band', 'netS_band', 'netR_band', 'ipc', 'L2_MPKI', 'L1I_MPKI', 'L3_MPKI'] 
