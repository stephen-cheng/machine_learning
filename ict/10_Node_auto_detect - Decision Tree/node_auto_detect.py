	...
	if node == []:
		...
		for feature in feature_pca_os:
			...
			if labels_knn != []:
				...
				print "Disparity of order of magnitude (log10): ", outlier_node_knn, outlier_value_knn
			...
			if labels_knn != []:
				...
				file.write("Disparity of order of magnitude (log10): "+"\r\n")
				file.write(" ".join(outlier_node_knn)+"\r\n")
				file.write(str(outlier_value_knn)+"\r\n")
			file.close()
		...
		if len(node) <= 2:
			...
			## print results into file
			file = open('file/%s_results.txt' % (stage_id),'a')
			file.write("Less than two nodes can not calculate relative entropy !"+"\r\n")
			file.close()
			...
			for feature in pca_list_pca:
				...
				if feature in feature_list_os:
					...
					for i in node:
						re_value_dict[i] = re_value_list[node.index(i)]
					#print '%s Relative Entropy of %s with each other are: ' % (feature, node), re_value_dict
					#print '%s Relative Entropy mean value of %s between other nodes are: ' % (feature, node), re_value_avg_list
			
					## print results into file
					file = open('file/%s_results.txt' % (stage_id),'a')
					file.write('%s Relative Entropy of %s with each other are: ' % (feature, node)+"\r\n")
					file.write(str(re_value_dict)+"\r\n")
					file.write('%s Relative Entropy mean value of %s between other nodes are: ' % (feature, node)+"\r\n")
					file.write(str(re_value_avg_list)+"\r\n")
					file.close()
					
					for re in re_value_avg_list:
						if re > re_threshold:
							re_outlier_value.append(re)
							re_outlier_node.append(node[re_value_avg_list.index(re)])			
					if re_outlier_value == []:
						print "Relative entropy can not find any abnormal node of %s !" % feature
						
						## print results into file
						file = open('file/%s_results.txt' % (stage_id),'a')
						file.write("Relative entropy can not find any abnormal node of %s !" % feature+"\r\n")
						file.close()
							
					else:
						print "Relative entropy found abnormal nodes of %s: " % feature, re_outlier_node, re_outlier_value
						
						## print results into file
						file = open('file/%s_results.txt' % (stage_id),'a')
						file.write("Relative entropy found abnormal nodes of %s: " % feature+"\r\n")
						file.write(" ".join(re_outlier_node)+"\r\n")
						file.write(str(re_outlier_value)+"\r\n")
						file.close()	
